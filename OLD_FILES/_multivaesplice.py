...
class MULTIVAESPLICE(BaseModuleClass):
    ...
    def inference(
        ...
        if self.n_input_genes == 0:
            x_rna = torch.zeros(x.shape[0], 1, device=x.device, requires_grad=False)#maybe we can change this to use layers instead...?
        else:
        ...
        mask_spl = x_junc.sum(dim=1) > 0 #making sure none of the splicing junctions get removed. 
    ...
    def get_reconstruction_loss_splicing(self, x, atse_counts, junc_counts, p, loss_type="beta_binomial", concentration=None):
        """
        Compute the reconstruction loss for splicing data using either a binomial
        or a beta‐binomial log-likelihood. Entries where atse_counts = 0 (corresponding with NaN junc ratio values)
        will be ignored (excluded) from the loss computation.
        
        Parameters
        ----------
        x : Tensor
            Input expression tensor (unused in this function but kept for consistency).
        atse_counts : Tensor
            Total count per gene (or junction); for beta binomial, used as the number
            of trials. Entries of zero are ignored in the loss.
        junc_counts : Tensor
            Observed count per junction (or “successes”).
        p : Tensor
            The probability output from the decoder.
        loss_type : str, optional
            Which loss to use. Options are "binomial" (default) or "beta_binomial".
        concentration : Tensor or float, optional
            For the beta-binomial parameterization, this controls dispersion.
            If not provided, a default value of 1.0 is used.
        
        Returns
        -------
        reconstruction_loss : Tensor
            The negative log-likelihood loss. Entries with atse_counts = 0
            are excluded from the calculation.
        """
        import torch

        # Create a mask to ignore entries where atse_counts = 0
        mask = atse_counts != 0

        if loss_type == "binomial":
            # Binomial log-likelihood calculation
            log_probabilities = torch.log(p + 1e-10)# Avoid log(0)
            log_complement_probabilities = torch.log(1 - p + 1e-10)
            ...
            # Only compute the loss over masked entries
            log_likelihood_masked = log_likelihood[mask]
            reconstruction_loss = -log_likelihood_masked.mean()

            return reconstruction_loss

        elif loss_type == "beta_binomial":
            # Use a custom beta binomial log-likelihood
            if concentration is None:
                concentration = torch.tensor(1.0, device=p.device)

            # Calculate alpha and beta from p and the concentration
            alpha = p * concentration
            beta = (1 - p) * concentration

            # Calculate log PMF using the beta-binomial formula
            # log PMF = lgamma(n+1) - lgamma(k+1) - lgamma(n-k+1)
            ...
            reconstruction_loss = -log_pm_masked.mean()
