...
#    sbatch --export=LATENT_DIM=64,LR=5e-4 run_pipeline.sh
#    (exported vars take precedence)
# ───────────────────────────────────────────────────────────────────────────

# Required
#MUDATA_PATH="/path/to/aligned__ge_splice_combined.h5mu"
#MUDATA_PATH="/path/to/SUBSETTOP5CELLSTYPES_aligned__.h5mu"
# Required training MuData
MUDATA_PATH="${MUDATA_PATH:-/gpfs/commons/groups/knowles_lab/Karin/Leaflet-analysis-WD/MOUSE_SPLICING_FOUNDATION/MODEL_INPUT/052025/train_70_30_ge_splice_combined_20250513_035938.h5mu}"
# Required test MuData
TEST_MUDATA_PATH="${TEST_MUDATA_PATH:-/gpfs/commons/groups/knowles_lab/Karin/Leaflet-analysis-WD/MOUSE_SPLICING_FOUNDATION/MODEL_INPUT/052025/test_30_70_ge_splice_combined_20250513_035938.h5mu}"

# Optional hyperparams for MODEL INIT (uncomment to override; defaults):
#N_GENES="None"                 # --n_genes (default: None)
#N_JUNCTIONS="None"             # --n_junctions (default: None)
#MODALITY_WEIGHTS="cell"        # --modality_weights (default: "equal")
#MODALITY_PENALTY="Jeffreys"    # --modality_penalty (default: "Jeffreys")
#N_HIDDEN="None"                # --n_hidden (default: None)
LATENT_DIM=20                     # --n_latent (default: None)
...
# Optional hyperparams for TRAINING (uncomment to override; defaults):
...
# UMAP labels (uncomment to override; default: broad_cell_type dataset)
...
RUN_DIR="${RUN_ROOT}/${BASE_NAME}_${TS}_$$_"
LATENT_EVAL_SUBDIR="latent_eval"
IMPUTE_EVAL_SUBDIR="imputation_eval"
mkdir -p "${RUN_DIR}/models" "${RUN_DIR}/figures" \
         "${RUN_DIR}/${LATENT_EVAL_SUBDIR}" "${RUN_DIR}/${LATENT_EVAL_SUBDIR}/figures" "${RUN_DIR}/${LATENT_EVAL_SUBDIR}/csv_files" \
         "${RUN_DIR}/${IMPUTE_EVAL_SUBDIR}"
...
# Run train.py with args
echo "=== Starting training ==="
python /gpfs/commons/home/svaidyanathan/repos/multivi_tools_splicing/multivi_splice_utils/runfiles/train_multivisplice.py "${args[@]}"

# === 2.  Latent space evaluation ===
# Build args for evaluation
eval_args=(--mudata_path "$MUDATA_PATH" --model_path "${RUN_DIR}/models" --out_dir "${RUN_DIR}/${LATENT_EVAL_SUBDIR}")
[ -n "$CLUSTER_NUMBERS" ]   && eval_args+=(--cluster_numbers $CLUSTER_NUMBERS)
[ -n "$NEIGHBOR_K" ]        && eval_args+=(--neighbor_k $NEIGHBOR_K)
[ -n "$CELL_TYPE_COLUMN" ]  && eval_args+=(--cell_type_column "$CELL_TYPE_COLUMN")
[ -n "$TOP_N_CELLTYPES" ]   && eval_args+=(--top_n_celltypes $TOP_N_CELLTYPES)


echo "=== Starting latent space evaluation ==="
python /gpfs/commons/home/svaidyanathan/repos/multivi_tools_splicing/multivi_splice_utils/runfiles/latentspace_multivisplice.py "${eval_args[@]}"

imp_args=(--test_mudata_path "$TEST_MUDATA_PATH" --model_path "${RUN_DIR}/models" --out_dir "${RUN_DIR}/${IMPUTE_EVAL_SUBDIR}")
echo "=== Starting imputation evaluation ==="
python imputation_eval.py "${imp_args[@]}"
