...
def main():
    ...
    print("Prepare an array to store the original junc_ratio for masked entries", flush=True)
    ...
    print("Track unique ATSEs and junctions masked, and record one example", flush=True)
    masked_atse_set   = set()
    masked_junc_set   = set()
    ...
    if example_record:
        ...
        print(f"  Original junc_ratio at that position: "
              f"{jr_original[cell_i, junc_i]}", flush=True)
        print(f"  After masking, junc_ratio:              "
              f"{jr_arr[cell_i, junc_i]}", flush=True)
        print(f"  Stored in 'junc_ratio_masked_original': "
              f"{orig_jr[cell_i, junc_i]}", flush=True)
