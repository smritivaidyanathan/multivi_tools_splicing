...
# Now sweeps over: code_dim, latent_dim, num_weight_vectors, and lr.
...
MAX_EPOCHS=750
LR=1e-5                      # baseline value; will be overridden by LRS loop
BATCH_SIZE=256
...
# Sweep lists
CODE_DIMS=(64)
LATENT_DIMS=(10)          # ← add/adjust as needed
LRS=("1e-5")   # ← add/adjust as needed
  ...
  for LATENT_DIM in "${LATENT_DIMS[@]}"; do
    for LR in "${LRS[@]}"; do
      for i in "${!ENCODER_TYPES[@]}"; do
        ...
        # Decide which values of NUM_WEIGHT_VECTORS to sweep
        if [[ "$ENCODER_TYPE" == "PartialEncoderWeightedSumEDDIMultiWeight" || \
          ...
          SWEEP_WEIGHTS=("")   # single pass with no flag
        fi

        for NUM_WEIGHT_VECTORS in "${SWEEP_WEIGHTS[@]}"; do
          # Only PartialEncoderImpute needs this flag
          ...
          JOB_NAME="sweep_SIMULATED_cd${CODE_DIM}_ld=${LATENT_DIM}_lr=${LR}_${i}_${FORWARD_STYLE}_${ENCODER_TYPE}"
...
echo "→ All sweep jobs (code_dim ∈ ${CODE_DIMS[*]}, latent_dim ∈ ${LATENT_DIMS[*]}, lr ∈ ${LRS[*]}) submitted."
